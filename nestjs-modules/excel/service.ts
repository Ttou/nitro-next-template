...
import type { ExcelModuleOptions, IExcelFileOptions } from './interface'
...
export class ExcelService {
  ...
  private options: ExcelModuleOptions
  ...
  private async createWorkbook(stream: PassThrough, cls: ClassConstructor<any>, data: any[]) {
    ...
    const ws = wb.addWorksheet(sheetName || this.defaultSheetName!, sheetOptions)
  ...
  private getStreamableFileOptions(cls: ClassConstructor<any>): StreamableFileOptions {
    ...
    return {
      disposition: `attachment; filename=${encodeURIComponent(fileName || this.defaultFileName!)}`,
      type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
    }
  }

  private get defaultSheetName() {
    return isFunction(this.options.defaultSheetName) ? this.options.defaultSheetName() : this.options.defaultSheetName
  }

  private get defaultFileName() {
    return isFunction(this.options.defaultFileName) ? this.options.defaultFileName() : this.options.defaultFileName
  }
}
